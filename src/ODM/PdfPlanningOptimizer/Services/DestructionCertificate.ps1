...
function New-DestructionCertificate {
    ...
    # Date de passage : remonter ~5 px vs 80.77 (Y vers le haut = Ymm diminue).
    [double]$visitDateBaselineYmm = 80.77 - (5.0 * 25.4 / [double]$script:CdsCertRasterDpi)
    # Ligne « Article » (PapierConfidentiel + prestation) : descendre ~8 px vs 87.67.
    [double]$articleRowBaselineYmm = 87.67 + (8.0 * 25.4 / [double]$script:CdsCertRasterDpi)
    [double]$prestationBesideLabelYmm = $articleRowBaselineYmm
    [double]$prestationBesideLabelOldYmm = 87.67
    [double]$prestationBesideLabelNewYmm = $prestationBesideLabelYmm
    ...
    $certLayout = @{
        ...
        VisitDate                     = @{ Xmm = 43.81; Ymm = $visitDateBaselineYmm }
        PapierConfidentiel            = @{ Xmm = 29.79; Ymm = $articleRowBaselineYmm }
        # Meme baseline que la ligne modèle « Article » (PapierConfidentiel) ; X = saisie à droite.
        PrestationPapierConfidentiel  = @{ Xmm = 72.0; Ymm = $prestationBesideLabelNewYmm }
