...
function Convert-XlsxToPdf {
    ...
    $sw.Stop()
    if ($ok) {
        script:Write-CnsXlsxToPdfLog -Message ("Conversion reussie via {0} en {1} ms" -f $engine, $sw.ElapsedMilliseconds) -Level 'INFO'
    }
    else {
        script:Write-CnsXlsxToPdfLog -Message ("Conversion echouee via {0} apres {1} ms" -f $engine, $sw.ElapsedMilliseconds) -Level 'WARN'
    }
    return $ok
}
