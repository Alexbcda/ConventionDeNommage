...
function Initialize-AppConfig {
    <#
    .SYNOPSIS
        Table clé/valeur pour la configuration applicative (SharePoint, version, centre, etc.).
    .NOTES
        Appelée dans Invoke-DatabaseMigrations et dans Get/Set-AppConfig (idempotent).
    #>
    param(
        [Parameter(Mandatory = $true)]
        $Connection
...
"@
        ...
        $null = Initialize-CalendarIndexTable -Connection $conn
        $null = Initialize-AppConfig -Connection $conn
...
function Get-AppConfig {
    <#
    .SYNOPSIS
        Récupère une valeur de configuration depuis la table AppConfig.
    .PARAMETER Key
        Clé de configuration (ex: "SharePointApiUrl", "AppVersion")
    .OUTPUTS
        [string] Valeur associée, ou $null si la clé n'existe pas.
    #>
    param([Parameter(Mandatory = $true)][string]$Key)
...
function Set-AppConfig {
    <#
    .SYNOPSIS
        Enregistre ou met à jour une valeur de configuration dans la table AppConfig.
    .PARAMETER Key
        Clé de configuration (ex: "SharePointApiUrl", "AppVersion")
    .PARAMETER Value
        Valeur à stocker (chaîne vide si $null)
    #>
    param(
        [Parameter(Mandatory = $true)][string]$Key,
        [AllowNull()][string]$Value = ''
