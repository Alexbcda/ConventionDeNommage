...
function Initialize-ApplicationWinForms {
    ...
    $doTrace = ($Trace -or ($env:CN_WINFORMS_TRACE -eq '1' -or $env:CN_WINFORMS_TRACE -eq 'true')) -and `
        (-not ($global:SuppressConsoleOutput -eq $true) -or $env:CN_VERBOSE -in @('1', 'true', 'TRUE', 'yes', 'YES'))
