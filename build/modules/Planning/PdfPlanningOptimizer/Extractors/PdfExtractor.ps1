...
function script:Get-ResolvedPdfToTextPath {
    ...
    foreach ($p in @(
            ...
            (Join-Path $repoRoot 'runtime\poppler\pdftotext.exe'),
    ...
    $runtimePopplerRoot = Join-Path $repoRoot 'runtime\poppler'
    if (Test-Path -LiteralPath $runtimePopplerRoot -PathType Container) {
        foreach ($d in @(Get-ChildItem -LiteralPath $runtimePopplerRoot -Directory -ErrorAction SilentlyContinue)) {
            [void]$candidates.Add((Join-Path $d.FullName 'Library\bin\pdftotext.exe'))
            [void]$candidates.Add((Join-Path $d.FullName 'bin\pdftotext.exe'))
        }
        $bundledExe = Get-ChildItem -LiteralPath $runtimePopplerRoot -Recurse -Filter 'pdftotext.exe' -File -ErrorAction SilentlyContinue |
            Select-Object -First 1
        if ($null -ne $bundledExe) {
            $candidates.Insert(0, $bundledExe.FullName)
        }
    }
    ...
    try {
        ...
        if (-not $pdftotext) {
            ...
            if ($allowEmpty) {
                ...
                for ($ri = 0; $ri -lt 4; $ri++) { $installRoot = Split-Path -Parent $installRoot }
                throw @"
[FATAL] pdftotext introuvable

Solutions :
1. Installer Poppler (recommandé) : https://github.com/oschwartz10612/poppler-windows/releases
2. Définir PDFTOTEXT_PATH = chemin\vers\pdftotext.exe
3. Copier pdftotext.exe (et ses DLL) dans runtime\poppler\ du package ou de l'installation
4. Mettre pdftotext.exe dans le PATH + PDFTOTEXT_ALLOW_PATH=1

Debug :
- PATH = $env:PATH
- PDFTOTEXT_PATH = $env:PDFTOTEXT_PATH
- PDFTOTEXT_ALLOW_PATH = $env:PDFTOTEXT_ALLOW_PATH
- runtime\poppler attendu sous : $(Join-Path $installRoot 'runtime\poppler')
"@
            }
        }
        else {
