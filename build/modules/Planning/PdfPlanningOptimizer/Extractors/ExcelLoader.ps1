...
function script:Get-BundledImportExcelModulePath {
    try {
        $installRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..\..\..\..') -ErrorAction Stop).Path
        $psd1 = Join-Path $installRoot 'runtime\ImportExcel\ImportExcel.psd1'
...
function script:Ensure-PlanningCompressionAssemblies {
    ...
    foreach ($assemblyName in $assemblyNames) {
        try {
            ...
            Write-Host ("[ExcelLoader] {0} indisponible : {1}" -f $assemblyName, $_.Exception.Message) -ForegroundColor Yellow
        }
    }
    ...
    foreach ($dllPath in @(
        ...
        try {
            ...
            Write-Host ("[ExcelLoader] Chargement {0} echoue : {1}" -f $dllPath, $_.Exception.Message) -ForegroundColor Yellow
        }
    }

    throw '[ExcelLoader] System.IO.Compression.ZipFile indisponible (assembly System.IO.Compression.FileSystem requis pour lire les fichiers .xlsx/.xlsm).'
}

function script:Ensure-ImportExcelModule {
    <#
    .SYNOPSIS
        Charge le module ImportExcel ; priorite au bundle runtime/, puis modules systeme.
    #>
    Write-Host '[ExcelLoader] Chargement du module ImportExcel...' -ForegroundColor Yellow
    script:Ensure-PlanningCompressionAssemblies

    if (Get-Module -Name ImportExcel -ErrorAction SilentlyContinue) {
        Write-Host '[ExcelLoader] ImportExcel deja charge' -ForegroundColor Green
        return
    }

    $bundledPsd1 = script:Get-BundledImportExcelModulePath
    Write-Host ("[ExcelLoader] Runtime path : {0}" -f $(if ($bundledPsd1) { $bundledPsd1 } else { '(introuvable)' })) -ForegroundColor DarkGray
    if ($bundledPsd1) {
        try {
            Import-Module -Name $bundledPsd1 -Force -ErrorAction Stop
            if (Get-Command Open-ExcelPackage -ErrorAction SilentlyContinue) {
                Write-Host '[ExcelLoader] ImportExcel charge depuis runtime' -ForegroundColor Green
                return
            }
        }
        catch {
            Write-Host ("[ExcelLoader] ImportExcel runtime echoue : {0}" -f $_.Exception.Message) -ForegroundColor Yellow
        }
    }

    if (-not (Get-Module -Name ImportExcel -ListAvailable -ErrorAction SilentlyContinue)) {
        Write-Host '[ExcelLoader] Recherche dans PSGallery...' -ForegroundColor Yellow
        try {
            Install-Module -Name ImportExcel -Scope CurrentUser -Force -ErrorAction Stop
        }
        catch {
            $msg = @"
[ExcelLoader] Le module ImportExcel est requis pour lire les fichiers .xlsx / .xlsm / .xltx sans Excel installe.

Le package devrait contenir runtime/ImportExcel/ (reconstruire le package sur une machine de build avec ImportExcel installe).
Sinon, installez-le une fois (connexion Internet) :
...
"@
    ...
    if (-not (Get-Command Open-ExcelPackage -ErrorAction SilentlyContinue)) {
        throw "[ExcelLoader] La commande Open-ExcelPackage est introuvable apres import du module. Mettez a jour : Update-Module ImportExcel"
    }
    Write-Host '[ExcelLoader] ImportExcel charge depuis modules systeme / PSGallery' -ForegroundColor Green
}
