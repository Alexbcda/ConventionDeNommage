...
function Resolve-PdfTextNormalizerScript {
    foreach ($name in @('PdfTextNormalizer.ps1', 'CnsPdfTextNormalizer.ps1', 'CnsPdfNorm.ps1')) {
        $path = Join-Path $PSScriptRoot $name
        if (Test-Path -LiteralPath $path) { return $path }
    }
    throw "Script PdfTextNormalizer introuvable dans $PSScriptRoot"
}

. (Resolve-PdfTextNormalizerScript)
...
function script:Extract-TrailingClientNameFromAddressHead {
    ...
    $m = [regex]::Match(
        $Text,
        '(?i)\s+((?:[A-Z\u00C0-\u00D6\u00D8-\u00DE\u00E0-\u00F6\u00F8-\u00FF]{2,})(?:\s+[A-Z\u00C0-\u00D6\u00D8-\u00DE\u00E0-\u00F6\u00F8-\u00FF0-9]{2,})*)\s*$'
    )
    if ($m.Success) {
...
function script:Split-MonolithicClientNameLayoutLines {
    ...
    foreach ($raw in @($Lines)) {
        ...
        $prestationLine = $beforeDate.TrimEnd(' ', '-', [char]0x2013, [char]0x2014, ',', ';')
        $nameLine = $afterDate.TrimStart(' ', '-', [char]0x2013, [char]0x2014, ',', ';').Trim()
