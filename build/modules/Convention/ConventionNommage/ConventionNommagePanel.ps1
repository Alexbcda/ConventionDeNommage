...
function Show-ConventionNommagePanel {
    ...
    $resolvedPdf = $FichierPDF
    if (-not [string]::IsNullOrWhiteSpace($FichierPDF) -and (Test-Path -LiteralPath $FichierPDF -PathType Leaf)) {
        try { $resolvedPdf = (Resolve-Path -LiteralPath $FichierPDF -ErrorAction Stop).Path }
        catch { $resolvedPdf = $FichierPDF }
    }
    $panel.Tag = @{
        FichierPDF  = $resolvedPdf
        RefDate     = $refDate
        RenamingNow = $false
    }
    ...
    $lblPdfFile = [System.Windows.Forms.Label]::new()
    $lblPdfFile.Name = 'lblPdfFile'
    if (-not [string]::IsNullOrWhiteSpace($resolvedPdf) -and (Test-Path -LiteralPath $resolvedPdf -PathType Leaf)) {
        $lblPdfFile.Text = ('Fichier : {0}' -f (Split-Path -Leaf $resolvedPdf))
        $lblPdfFile.ForeColor = [System.Drawing.Color]::DarkGreen
    }
    else {
        $lblPdfFile.Text = 'Fichier : (aucun PDF associe — utilisez le clic droit sur un .pdf)'
        $lblPdfFile.ForeColor = [System.Drawing.Color]::DarkRed
    }
    $lblPdfFile.Font = $script:PoliceNormal
    $lblPdfFile.Location = [System.Drawing.Point]::new(0, 52)
    $lblPdfFile.Size = [System.Drawing.Size]::new(900, 22)
    $panel.Controls.Add($lblPdfFile)
    ...
    $lblCollecte.Location = [System.Drawing.Point]::new(0, 98)
    ...
    $txtCollecte.Location = [System.Drawing.Point]::new(0, 125)
    ...
    $btnCert.Location = [System.Drawing.Point]::new(0, 190)
    ...
    $lblModeForce.Location = [System.Drawing.Point]::new(260, 265)
