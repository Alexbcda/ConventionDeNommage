...
function Ensure-PackagePs1Utf8Bom {
    ...
    if ($fixed -gt 0) {
        Write-BuildWarn "UTF-8 BOM ajoute a $fixed fichier(s) .ps1 (compatibilite PowerShell 5.1)"
    }
    else {
        Write-BuildSuccess 'Encodage UTF-8 BOM deja present sur les .ps1'
    }
}
...
foreach ($fileName in @('ASSISTANT.bat', 'INSTALL.bat', 'install_assistant.ps1', 'install_gui.ps1', 'fix_installation.ps1', 'clean_install.ps1', 'uninstall_assistant.ps1', 'Register-AssistantContextMenu.ps1')) {
    $src = Join-Path $repoRoot $fileName
    if (-not (Test-Path -LiteralPath $src)) {
        $src = Join-Path $repoRoot ("Tools\{0}" -f $fileName)
    }
    if (Test-Path -LiteralPath $src) {
        Copy-Item -LiteralPath $src -Destination (Join-Path $outputPath $fileName) -Force
    }
}
...
Ensure-PackagePs1Utf8Bom -PackageRoot $outputPath
...
Package  : $outputPath
Version  : $Version
Cle USB  : copier tout le dossier package, puis double-clic INSTALL.bat
Install  : .\package\install_assistant.ps1 -Centre fontaine -Quiet -InstallDir `"`$env:LOCALAPPDATA\ASSISTANT`"
Corriger : .\fix_installation.ps1 -Centre fontaine
