# AffectationNbTourneesPanel.ps1 - Interface utilisateur pour définir le nombre de tournées

# Fonction de secours
function Save-NbTournees {
    param([int]$NbTournees)
    $script:NbTournees = $NbTournees
    Write-Host "[METIER] Nombre de tournées sauvegardé: $NbTournees" -ForegroundColor Green
}
...
function Show-AffectationNbTourneesPanel {
    ...
    $btnValider.Add_Click({
        ...
        # Vérification si la valeur est vide ou nulle
        if ($nbTournees -le 0) {
